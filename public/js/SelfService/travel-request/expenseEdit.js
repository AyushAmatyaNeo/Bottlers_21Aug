(function ($, app) {
    ...
    angular.module('hris', [])
        .controller('expenseDtlController', function ($scope, $http, $window) {
            ...
            var allCategoryType = document.allCategoryType;
            ...
            $scope.categoryTypeList = TravelClass;
                    ...
                    $scope.$apply(function () {
                        ...
                        if (expenseDtlNum > 0) {
                            ...
                            for (var j = 0; j < expenseDtlNum; j++) {
                                ...
                                let tempCategory = allCategoryType.findIndex(record => record.VALUE === expenseDtlList[j].CATEGORY_NAME);
                                console.log(allCategoryType[tempCategory]);
                                $scope.expenseDtlFormList.push(angular.copy({
                                    ...
                                    categoryType: TravelClass,
                                    category: allCategoryType[tempCategory].KEY,
                                    fare: parseFloat(expenseDtlList[j].FARE),
                                    localConveyence: parseFloat(expenseDtlList[j].LOCAL_CONVEYENCE),
                                    allowance: dailyAllowance,
                                    twentyFivePercent: parseFloat(expenseDtlList[j].CATEGORY_AMOUNT),
            ...
            $scope.total = function (fare, localConveyence, miscExpense, twentyFivePercent, standardExchangeRate, exchangeRate) {
                ...
                var total = (fare1 + localConveyence1 + miscExpense1) * rate + twentyFivePercent;
                // console.log(total);
            ...
            $scope.sumAllTotal = function (list) {
                ...
                angular.forEach(list, function (item) {
                    var total1 = $scope.total(item.fare, item.localConveyence, item.miscExpense, item.twentyFivePercent, item.standardExchangeRate, item.exchangeRate);
                    // console.log(total1);
