...
ORDER BY UPPER(HG.GENDER_NAME)";
  ...
  public function turnOverReport($data)
  {
    ...
    company_name,Beginning,Ending,Real_Separation,((Beginning+Ending)/2) as average from(SELECT
    company_code,
    company_name,
...
FROM(
      select c.company_code,c.company_name ,
        ...
        hris_company c where  (e.company_id=c.company_id)  and (e.employee_id=he.employee_id) and (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE  IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL)
        and he.end_date  between $monthYrsCondition1 and $monthYrsCondition2 {$searchCondition['sql']} and e.status='E'
